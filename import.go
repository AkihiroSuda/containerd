...
func resolveOCIIndex(idx ocispec.Index, refObject string) (ocispec.Descriptor, error) {
	...
	if tag == "" && dgst == "" {
		return ocispec.Descriptor{}, errors.Errorf("unexpected object: %q", refObject)
	}
...
func (c *Client) importFromOCITar(ctx context.Context, ref string, iopts importOpts) (Image, error) {
	tr := tar.NewReader(iopts.reader)
	store := c.ContentStore()
