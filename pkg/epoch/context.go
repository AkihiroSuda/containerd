...
const GRPCHeader = "source-date-epoch"
...
func WithSourceDateEpoch(ctx context.Context, tm *time.Time) context.Context {
	ctx = context.WithValue(ctx, epochKey{}, tm)
	var tmString string
	if tm != nil {
		tmString = fmt.Sprintf("%d", tm.Unix())
	}
	return grpcctx.WithGRPCNamespaceHeader(ctx, GRPCHeader, tmString)
}
...
func FromContext(ctx context.Context) *time.Time {
	...
	if v == nil {
		if s, ok := grpcctx.FromGRPCHeader(ctx, GRPCHeader); ok && s != "" {
