...
func testFetch(ctx context.Context, f remotes.Fetcher, desc ocispec.Descriptor) error {
	...
	r2, sz2, err := fByDigest.FetchByDigest(ctx, desc.Digest)
	if err != nil {
	...
	if sz2 != desc.Size {
		...
		if err != nil {
			return fmt.Errorf("FetcherByDigest: size mismatch: %d != %d (content: %v)", sz2, desc.Size, err)
		}
		return fmt.Errorf("FetcherByDigest: size mismatch: %d != %d (content: %q)", sz2, desc.Size, string(r2b))
	}
