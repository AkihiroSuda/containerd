...
func lookupBinary(t *testing.T) string {
	...
	for _, c := range candidates {
		...
		if err == nil {
			t.Logf("found docker registry binary: %q", bin)
			return bin
		}
	}
...
func New(t *testing.T, verbose bool) *Registry {
	bin := lookupBinary(t)
...
// Start starts the registry and sets r.url.
// This function blocks until the registry starts serving.
func (r *Registry) Start(t *testing.T) {
	// recent registry writes the log to stdout, but some older version seems using stderr
	...
	defer cancel()
	r.url = detectURL(ctx, t, rc, r.verbose)
}

func detectURL(ctx context.Context, t *testing.T, rc io.ReadCloser, verbose bool) string {
