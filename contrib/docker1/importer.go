...
type Importer struct {
	Prefix string // mandatory atm, may change in future
}

var _ imageformats.Importer = &Importer{}
...
func (importer *Importer) Import(ctx context.Context, store content.Store, reader io.Reader) ([]images.Image, error) {
	...
	if len(mfsts) > 1 {
		// FIXME(AkihiroSuda)
		return nil, errors.New("multi-image archive is not supported yet")
	}
	mfst := mfsts[0]
	if mfst.Parent != "" {
	...
	for _, repoTag := range mfst.RepoTags {
		imgrecs = append(imgrecs, images.Image{
			Name:   importer.Prefix + repoTag,
			Target: *desc,
		})
	}
	return imgrecs, nil
