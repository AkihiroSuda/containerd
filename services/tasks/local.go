...
func (l *local) writeContent(ctx context.Context, mediaType, ref string, r io.Reader) (*types.Descriptor, error) {
	writer, err := l.store.Writer(ctx, ref, ocispec.Descriptor{MediaType: mediaType})
	if err != nil {
		return nil, err
	}
	defer writer.Close()
