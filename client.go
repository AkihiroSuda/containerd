...
func WithImporter(importer imageformats.Importer) ImportOpt {
	return func(c *importOpts) error {
		if c.importer != nil {
			return errors.New("importer already set")
		}
		c.importer = importer
		return nil
	}
...
func resolveImportOpt(opts ...ImportOpt) (importOpts, error) {
	...
	// TODO(AkihiroSuda): use OCI as the default importer, but importer interface not fixed yet
	if iopts.importer == nil {
		return iopts, errors.New("importer not set")
	}
	return iopts, nil
}
...
func (c *Client) Import(ctx context.Context, reader io.Reader, opts ...ImportOpt) ([]Image, error) {
	iopts, err := resolveImportOpt(opts...)
	if err != nil {
		return nil, err
	}
	imgrecs, err := iopts.importer.Import(ctx, c.ContentStore(), reader)
	if err != nil {
		return nil, err
	}
	var images []Image
