...
func applyNaive(ctx context.Context, root string, tr *tar.Reader, options ApplyOptions) (size int64, err error) {
	st := &applyNaiveState{
		unpackedPaths: make(map[string]struct{}, 0),
		aufsHardlinks: make(map[string]*tar.Header, 0),
	}
	...
	for {
		...
		skipped, err := applyNaive1(ctx, root, tr, options, st, hdr)
		if skipped {
			continue
		}
		if err != nil && options.OnError != nil {
			err = options.OnError(hdr.Name, err)
		}
		if err != nil {
			return size, err
		}
...
// if skipped is true err must be nil
func applyNaive1(ctx context.Context, root string, tr *tar.Reader, options ApplyOptions, st *applyNaiveState, hdr *tar.Header) (skipped bool, err error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "failed to get root path")
	}
	...
	if strings.HasPrefix(hdr.Name, whiteoutMetaPrefix) {
		...
		if strings.HasPrefix(hdr.Name, whiteoutLinkDir) && hdr.Typeflag == tar.TypeReg {
			...
			st.aufsHardlinks[basename] = hdr
			if st.aufsTempdir == "" {
				if st.aufsTempdir, err = ioutil.TempDir("", "dockerplnk"); err != nil {
					return false, err
				}
				defer os.RemoveAll(st.aufsTempdir)
			}
			p, err := fs.RootPath(st.aufsTempdir, basename)
			if err != nil {
				return false, err
			}
	...
	if strings.HasPrefix(base, whiteoutPrefix) {
		...
		if base == whiteoutOpaqueDir {
			...
			err = filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
				...
				if _, exists := st.unpackedPaths[path]; !exists {
		...
		if !strings.HasPrefix(originalPath, dir) {
			return false, errors.Wrapf(errInvalidArchive, "invalid whiteout name: %v", base)
		}
	...
	if hdr.Typeflag == tar.TypeLink && strings.HasPrefix(filepath.Clean(hdr.Linkname), whiteoutLinkDir) {
		...
		srcHdr = st.aufsHardlinks[linkBasename]
		if srcHdr == nil {
			return false, fmt.Errorf("Invalid aufs hardlink")
		}
		p, err := fs.RootPath(st.aufsTempdir, linkBasename)
		if err != nil {
			return false, err
		}
	...
	if hdr.Typeflag == tar.TypeDir {
		st.dirs = append(st.dirs, hdr)
	}
	st.unpackedPaths[path] = struct{}{}
	return false, nil
}
